...
abstract class RemoteDataSource {
  ...
  Future<ApiResponse<StorageResponseModel>> uploadFile(
      {required OnUploadProgressCallback uploadProgressCallback,
      required File file});
...
class RemoteDataSourceImpl implements RemoteDataSource {
  ...
  Future<ApiResponse<StorageResponseModel>> uploadFile(
    ...
    final response = await httpClient.post("/upload",
      ...
      uploadProgressCallback(UploadProgress(
          totalSize: total,
          sendSize: uploaded,
          uploadedProgressData: uploadedPercentage));
    });
    ...
    return ApiResponse<StorageResponseModel>.error(
        errorMessage:
            response.data["error"]["message"] ?? "update_failed".tr());
  }
