...
export default {
    ...
    actions: {
        ...
        async sendMsgSetCookbookDenomMetadata({ rootGetters }, { value, fee = [], memo = '' }) {
            try {
                const txClient = await initTxClient(rootGetters);
                const msg = await txClient.msgSetCookbookDenomMetadata(value);
                const result = await txClient.signAndBroadcast([msg], { fee: { amount: fee,
                        gas: "200000" }, memo });
                return result;
            }
                ...
                if (e == MissingWalletError) {
                    throw new SpVuexError('TxClient:MsgSetCookbookDenomMetadata:Init', 'Could not initialize signing client. Wallet is required.');
                }
                else {
                    throw new SpVuexError('TxClient:MsgSetCookbookDenomMetadata:Send', 'Could not broadcast Tx: ' + e.message);
                }
            }
        },
        ...
        async MsgSetCookbookDenomMetadata({ rootGetters }, { value }) {
            try {
                const txClient = await initTxClient(rootGetters);
                const msg = await txClient.msgSetCookbookDenomMetadata(value);
                return msg;
                ...
                if (e == MissingWalletError) {
                    ...
                    throw new SpVuexError('TxClient:MsgSetCookbookDenomMetadata:Create', 'Could not create message: ' + e.message);
                }
            }
        },
    }
