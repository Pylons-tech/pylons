...
const baseStripePrice: object = { Amount: 0, Currency: '', Description: '', Images: '', Name: '', Quantity: 0 }

export const StripePrice = {
  encode(message: StripePrice, writer: Writer = Writer.create()): Writer {
    if (message.Amount !== 0) {
  ...
  decode(input: Reader | Uint8Array, length?: number): StripePrice {
    const reader = input instanceof Uint8Array ? new Reader(input) : input
    let end = length === undefined ? reader.len : reader.pos + length
    const message = { ...baseStripePrice } as StripePrice
    message.Images = []
    while (reader.pos < end) {
      const tag = reader.uint32()
      switch (tag >>> 3) {
...
const baseStripeInventory: object = { Quantity: 0, Type: '', Value: '' }

export const StripeInventory = {
  encode(message: StripeInventory, writer: Writer = Writer.create()): Writer {
    if (message.Quantity !== 0) {
  ...
  decode(input: Reader | Uint8Array, length?: number): StripeInventory {
    const reader = input instanceof Uint8Array ? new Reader(input) : input
    let end = length === undefined ? reader.len : reader.pos + length
    const message = { ...baseStripeInventory } as StripeInventory
    while (reader.pos < end) {
      const tag = reader.uint32()
      switch (tag >>> 3) {
        case 1:
