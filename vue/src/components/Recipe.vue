...
export default {
	...
	methods: {
		...
		async submitJSON() {
			...
			await this.$store.dispatch('Pylonstech.pylons.pylons/sendMsgCreateRecipe', {
				value: recipeObj,
				fee: [],
				memo: ''
			})
			console.log(
				(await this.$store.dispatch('Pylonstech.pylons.pylons/QueryListRecipesByCookbook', {
					params:{
						CookbookID : recipeObj.cookbookID
					}
				})).Recipes
			)
			this.$store.state.ListExecutionsByRecipe
		}
	},
	data() {
		return {
