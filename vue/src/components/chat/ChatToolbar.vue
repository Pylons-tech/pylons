	...
	<v-app-bar color="primary" dark app clipped-right>
		...
		<v-toolbar-items>
			...
			<v-menu offset-y origin="center center" class="elelvation-1" transition="scale-transition">
				...
				<NotificationList v-show="getNotification.length > 0" :items="getNotification" />
			</v-menu>
			<v-menu offset-y origin="center center" class="elelvation-1" transition="scale-transition">
				<template #activator="{ on }">
				...
				<v-list>
					<v-list-item-group v-model="$vuetify.lang.current">
						<v-list-item v-for="item in availableLanguages" :key="item.value" :value="item.value" @click="handleChangeLocale(item)">
			...
			<v-menu offset-y origin="center center" transition="scale-transition">
				<template #activator="{ on }">
					<v-btn slot="activator" icon large text v-on="on">
						<CAvatar :size="36" :username="getUsername" :src="getAvatar" :status="getUserStatus" />
					</v-btn>
				</template>
				<v-list class="pa-0">
					<v-list-item
