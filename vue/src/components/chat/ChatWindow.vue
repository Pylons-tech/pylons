<template>
	<div class="chat_window">
		...
		<v-card>
			<VuePerfectScrollbar ref="scrollbar" class="chat_window__scrollbar grey lighten-5" :style="computeHeight">
				<v-card-text class="messaging_list pa-3">
					<template v-for="(item, index) in getMessages">
						<ChatMessagingItem :key="index" :username="item.username" :text="item.text" :created-at="item.createdAt" />
					</template>
				</v-card-text>
