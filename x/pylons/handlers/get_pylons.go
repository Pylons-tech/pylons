...
func (srv msgServer) GetPylons(ctx context.Context, msg *types.MsgGetPylons) (*types.MsgGetPylonsResponse, error) {
	...
	err = srv.CoinKeeper.AddCoins(sdkCtx, requester, msg.Amount) // If so, deduct the Bid amount from the sender
	if err != nil {
...
func (srv msgServer) GoogleIAPGetPylons(ctx context.Context, msg *types.MsgGoogleIAPGetPylons) (*types.MsgGoogleIAPGetPylonsResponse, error) {
	err := msg.ValidateBasic()
	...
	if srv.HasGoogleIAPOrder(sdkCtx, msg.PurchaseToken) {
	...
	err = srv.RegisterGoogleIAPOrder(sdkCtx, iap)
	if err != nil {
	...
	err = srv.CoinKeeper.AddCoins(sdkCtx, requester, iap.GetAmount())
	if err != nil {
