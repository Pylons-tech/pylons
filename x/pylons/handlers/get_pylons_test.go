...
func TestHandlerMsgGetPylons(t *testing.T) {
	...
	for testName, tc := range cases {
		t.Run(testName, func(t *testing.T) {
			...
			if !tc.showError {
				require.True(t, keeper.HasCoins(tci.PlnK, tci.Ctx, tc.fromAddress, types.NewPylon(tc.reqAmount)))
				require.False(t, keeper.HasCoins(tci.PlnK, tci.Ctx, tc.fromAddress, types.NewPylon(tc.reqAmount+1)))
				require.True(t, keeper.HasCoins(tci.PlnK, tci.Ctx, tc.fromAddress, types.NewPylon(tc.reqAmount-1)))
			} else {
