...
func TestExecuteRecipeWeightedOutputs(t *testing.T) {
	net := network.New(t)
	val := net.Validators[0]
	ctx := val.ClientCtx
	cookbookID := "testCookbookID"
	...
	weightedOutputs, err := json.Marshal([]types.WeightedOutputs{
		{
			EntryIDs: []string{"testID"},
			Weight:   1,
		},
	...
	// check the recipe to see the weightedoutput
	expectedWeightedOutput := uint64(1)
	args = []string{cookbookID, recipeID}
	out, err = clitestutil.ExecTestCLICmd(ctx, cli.CmdShowRecipe(), args)
	require.NoError(t, err)
	var recipeResp types.QueryGetRecipeResponse
	require.NoError(t, ctx.Codec.UnmarshalJSON(out.Bytes(), &recipeResp))
	require.Equal(t, expectedWeightedOutput, recipeResp.Recipe.Outputs[0].Weight)
}
