...
// StripeOrder is a struct that contains all the metadata of a stripe order
...
func NewStripeOrder(ProductID, PaymentId, PaymentMethod, ReceiptDataBase64, Signature string, Sender sdk.AccAddress) StripeOrder {
	cb := StripeOrder{
		NodeVersion:       "0.0.1",
		ProductID:         ProductID,
		PaymentId:         PaymentId,
...
func (iap StripeOrder) GetAmount() sdk.Coins {
	...
	for _, giapProduct := range config.Config.StripeIAP {
