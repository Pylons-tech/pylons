...
class CustomTransactionSigningGateway {
  ...
  Future<Either<TransactionSigningFailure, SignedTransaction>> signTransaction({
    ...
    required AccountLookupKey walletLookupKey,
      ...
      _transactionSummaryUI
          ...
          .flatMap(
            (userAccepted) => _infoStorage.getPrivateCredentials(walletLookupKey).leftMap((err) => left(StorageProblemSigningFailure(err))),
          )
          .flatMap(
  ...
  Future<String> signPureMessage({required NetworkInfo networkInfo, required AccountLookupKey walletLookupKey, required String msg}) async {
    final retCreds = await _infoStorage.getPrivateCredentials(walletLookupKey);
    if (retCreds.isRight()) {
      final creds = retCreds.toOption().toNullable();
      if (creds != null) {
        final wallet = (creds as AlanPrivateAccountCredentials).alanAccount(networkInfo);
  ...
  Future<Either<TransactionBroadcastingFailure, TransactionResponse>> broadcastTransaction({
    ...
    required SignedTransaction transaction,
      ...
      _infoStorage.getPrivateCredentials(walletLookupKey).leftMap<TransactionBroadcastingFailure>((err) => left(StorageProblemBroadcastingFailure())).flatMap(
  ...
  Future<Either<AccountDerivationFailure, PrivateAccountCredentials>> deriveWallet({
    required AccountDerivationInfo walletDerivationInfo,
  }) async =>
      _findCapableDerivator(walletDerivationInfo).derive(accountDerivationInfo: walletDerivationInfo);

  Future<Either<CredentialsStorageFailure, List<AccountPublicInfo>>> getWalletsList() => _infoStorage.getAccountsList();

  /// Verifies if passed lookupKey is pointing to a valid wallet stored within the secure storage.
  Future<Either<TransactionSigningFailure, bool>> verifyLookupKey(AccountLookupKey walletLookupKey) => _infoStorage.verifyLookupKey(walletLookupKey);
  ...
  AccountDerivator _findCapableDerivator(AccountDerivationInfo walletDerivationInfo) => _derivators.firstWhere(
