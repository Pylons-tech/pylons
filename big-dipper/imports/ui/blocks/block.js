    ...
    render() {
        ...
        if (proposer){
            ...
            return <Row className="block-info">
                ...
                <Col xs={8} sm={3} md={2} lg={2} className="text-truncate"><Link to={"/validator/" + proposerOperatorAddress}><Avatar moniker={moniker} profileUrl={proposer.profile_url} address={proposerOperatorAddress} list={true} /> {moniker}</Link></Col>
                <Col xs={4} sm={1} md={homepage ? 2 : 2}><i className="fas fa-sync d-sm-none"></i> {numbro(this.props.block.transNum).format('0,0')}</Col>
