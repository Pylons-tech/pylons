...
void main() {
  group("$PracticeTest", () {
    ...
    setUp(() {
      ...
      when(repository.getMnemonic()).thenAnswer((realInvocation) async => const Right(MOCK_MNEMONIC));
    });
    tearDown(() {
      GetIt.I.reset();
    });
    ...
    testWidgets('should drag widget from top widget to empty bottom widget', (tester) async {
      ...
      final int bottomTargetIndex =
          practiceTestViewModel.givenListNotifier.value.indexWhere((element) => element.title == "laundry");
      final randomMnemonicFinder = find.text("laundry");
      final randomMnemonicTarget = find.byKey(const ValueKey('target_key_5'));
