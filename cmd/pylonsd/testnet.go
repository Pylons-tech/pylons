...
func testnetCmd(mbm module.BasicManager, genBalIterator banktypes.GenesisBalancesIterator) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a Pylonsd testnet",
...
Example:
	...
	cmd.Flags().String(flagNodeDaemonHome, "pylonsd", "Home directory of the node's daemon configuration")
...
func InitTestnet(
	...
	for i := 0; i < numValidators; i++ {
		...
		pylonsTokens := sdk.TokensFromConsensusPower(50000)
		coins := sdk.Coins{
			sdk.NewCoin(fmt.Sprintf("%stoken", nodeDirName), accTokens),
			sdk.NewCoin(pylonstypes.Pylon, pylonsTokens),
