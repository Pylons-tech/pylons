...
func TestStripeGetPylonsViaCLI(originT *originT.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			paymentMethod: "card",
			receiptData:   `{"orderId":"GPA.3376-6117-5921-78573","packageName":"com.pylons.loud","productId":"pylons_1000","paymentId":"pi_1DoShv2eZvKYlo2CqsROyFun","paymentMethod":"card","acknowledged":false}`,
			// Correct signature
	...
	for tcNum, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			msgStripeGetPylons := types.NewMsgStripeGetPylons(tc.productID, tc.paymentId, tc.paymentMethod, receiptDataBase64, tc.signature, getPylonsAccInfo.Address)
